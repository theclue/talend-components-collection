...
public class TalendFlowBacklinkBehaviour extends Observable implements TalendFlowBehaviour {
	...
	public TalendFlowBacklinkBehaviour(String apiKey, MajesticEndpoint endpoint, String token, MajesticDatasource datasource, String item) {
		...
		if (apiKey == null)
			throw new IllegalArgumentException(rb.getString("exception.nullApiKey"));
		if (token == null)
			throw new IllegalArgumentException(rb.getString("exception.nullToken"));
	...
	public void visit(TalendFlow target) throws IllegalStateException {
		ResourceBundle rb = ResourceBundle.getBundle("tMajesticBacklinkInput", Locale.getDefault());
		if (target == null)
			throw new IllegalArgumentException(rb.getString("exception.nullTargetFlow"));

		TalendRowFactory rowFactory = target.getModel().getRowFactory();
		TalendRow current;

		setChanged();
		notifyObservers(new MajesticLogger("USER_DEF_LOG", Thread.currentThread().getId(), "DEBUG", String.format(
				rb.getString("log.visitStart"), target.getName())));
		...
		if (maxSourceUrlRef > 0)
			parameters.put("MaxSourceURLsPerRefDomain", Integer.toString(maxSourceUrlRef));
		if (maxSameSourceUrl > 0)
			parameters.put("MaxSameSourceURLs", Integer.toString(maxSameSourceUrl));
		...
		do {
			...
			retryCount++;
			setChanged();
			notifyObservers(new MajesticLogger("USER_DEF_LOG", Thread.currentThread().getId(), "DEBUG", String.format("item to check: %s",
					parameters.get("item"))));
			response = api.executeOpenAppRequest("GetBackLinkData", parameters, token);

			if ((!response.isOK() && response.getErrorMessage().equals(rb.getString("exception.read.time.out.msg")))) {
				setChanged();
				notifyObservers(new MajesticLogger("USER_DEF_LOG", Thread.currentThread().getId(), "DEBUG", print(response)));
				retry = true;
			}
		...
		if (!response.isOK()) {
			...
			setChanged();
			notifyObservers(new MajesticLogger("USER_DEF_LOG", Thread.currentThread().getId(), "DEBUG", print(response)));
			throw new RuntimeException(retryCount == maxRetry ? "Majestic Server KO" : response.getErrorMessage());
		}
		...
		// log query metadata
		setChanged();
		notifyObservers(new MajesticLogger("USER_DEF_LOG", Thread.currentThread().getId(), "INFO", String.format(
				rb.getString("log.metadata"), results.getTableParams())));

		totalBacklinks = Integer.parseInt(response.getParamForName("TotalBackLinks") != null ? response.getParamForName("TotalBackLinks")
				: "0");
				...
				case DATE:
					...
					case DATE:
						try {
							...
							setChanged();
							notifyObservers(new MajesticLogger("USER_DEF_LOG", Thread.currentThread().getId(), "WARNING", String.format(
									rb.getString("warning.unparsableDate"), link.get("Date"))));
						}
						break;
					default:
						throw new IllegalArgumentException(String.format(rb.getString("exception.uncastableColumn"), row.getKey().getType()
				...
				case FIRST_INDEXED_DATE:
					...
					case DATE:
						try {
							...
							setChanged();
							notifyObservers(new MajesticLogger("USER_DEF_LOG", Thread.currentThread().getId(), "WARNING", String.format(
									rb.getString("warning.unparsableDate"), link.get("FirstIndexedDate"))));
						}
						break;
					default:
						throw new IllegalArgumentException(String.format(rb.getString("exception.uncastableColumn"), row.getKey().getType()
				...
				case LAST_SEEN_DATE:
					...
					case DATE:
						try {
							...
							setChanged();
							notifyObservers(new MajesticLogger("USER_DEF_LOG", Thread.currentThread().getId(), "WARNING", String.format(
									rb.getString("warning.unparsableDate"), link.get("LastSeenDate"))));
						}
						break;
					default:
						throw new IllegalArgumentException(String.format(rb.getString("exception.uncastableColumn"), row.getKey().getType()
				...
				case DATE_LOST:
					...
					case DATE:
						try {
							...
							setChanged();
							notifyObservers(new MajesticLogger("USER_DEF_LOG", Thread.currentThread().getId(), "WARNING", String.format(
									rb.getString("warning.unparsableDate"), link.get("DateLost"))));
						}
						break;
					default:
						throw new IllegalArgumentException(String.format(rb.getString("exception.uncastableColumn"), row.getKey().getType()
	...
	private String print(Response response) {
		return "Response [isOK()=" + response.isOK() + ", getCode()=" + response.getCode() + ", getErrorMessage()="
				+ response.getErrorMessage() + ", getFullError()=" + response.getFullError() + "]";
	}
