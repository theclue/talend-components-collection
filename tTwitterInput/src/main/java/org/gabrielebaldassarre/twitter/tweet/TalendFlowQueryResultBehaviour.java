...
public class TalendFlowQueryResultBehaviour extends Observable implements TalendFlowBehaviour, Iterable<TalendRow>{
	 ...
	 */
	public TalendFlowQueryResultBehaviour(Twitter client, Query q, int limit) {
		this.client = client;
		this.limit = limit;
		this.q = q;
		this.retrieved = 0;
	...
	/**
	 * Chech the validity of this visitor. It's false if a {@link TalendFlow} was not defined already as a valid output storage for search operations.
	 *
	 ...
	 * @throws IllegalStateException if target flow was not visited before trigging the search
	 * @throws IllegalStateException if no valid query object was passed to this visitor before trigging the search
	 */
